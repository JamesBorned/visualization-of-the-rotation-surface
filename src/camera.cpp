...
namespace mt
{
	...
	Pixel* Camera::Picture() { return m_picture; } // возвращает массив пикселей
	...
	void Camera::Clear() // очистить экран
	{
		Fill({ 0,0,0,0 });
	}
	...
	void Camera::ProjectPoint(Point p, Pixel c)
	{
		...
		P_res = R * P; // координаты точки М отн. камеры
		...
		{
			// if (m_picture->r == 255 ... солнце в окне
			m_picture[(int)v * m_width + (int)u] = c; // c - цвет
		}
	}
